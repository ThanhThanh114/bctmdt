        ...
        <div class="card">
            <div class="card-header">
                ...
                <div class="card-tools">
                    <a href="{{ route('bus-owner.tram-xe.create') }}" class="btn btn-primary btn-sm">
            ...
            <div class="card-body">
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        <form action="{{ route('bus-owner.tram-xe.index') }}" method="GET">
                    ...
                    <div class="col-md-6 text-right">
                        @if(request('search'))
                        <a href="{{ route('bus-owner.tram-xe.index') }}" class="btn btn-outline-secondary btn-sm">
                ...
                <div class="table-responsive">
                    <table class="table table-hover table-bordered" id="tramXeTable">
                        ...
                        <tbody>
                            ...
                            <tr data-id="{{ $tram->ma_tram_xe }}">
                                <td class="text-center">
                                    <span class="badge badge-primary">{{ $tram->ma_tram_xe }}</span>
                                </td>
                                <td>
                                ...
                                <td>
                                    @if($tram->nhaXe)
                                    <span class="badge badge-success" title="{{ $tram->nhaXe->ten_nha_xe }}">
                                        <i class="fas fa-building mr-1"></i>{{ $tram->nhaXe->ten_nha_xe }}
                                    </span>
                                    @else
                                ...
                                <td class="text-center">
                                    @if($tram->nhaXe && $tram->nhaXe->so_dien_thoai)
                                    <a href="tel:{{ $tram->nhaXe->so_dien_thoai }}" class="text-success" title="{{ $tram->nhaXe->so_dien_thoai }}">
                                        <i class="fas fa-phone"></i> {{ $tram->nhaXe->so_dien_thoai }}
                                    </a>
                                    @else
                                ...
                                <td class="text-center">
                                    <div class="btn-group">
                                        ...
                                        <a href="{{ route('bus-owner.tram-xe.edit', $tram->ma_tram_xe) }}"
                                            class="btn btn-sm btn-warning" title="Chỉnh sửa">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <button type="button"
                                            class="btn btn-sm btn-danger btn-delete"
                                            data-id="{{ $tram->ma_tram_xe }}"
                                            data-name="{{ $tram->ten_tram }}"
                                            title="Xóa">
                            ...
                            <tr id="noDataRow">
                                <td colspan="7" class="text-center text-muted py-5">
                                    ...
                                    <p>Không có trạm xe nào khớp với từ khóa: <strong>"{{ request('search') }}"</strong></p>
                                    <a href="{{ route('bus-owner.tram-xe.index') }}" class="btn btn-primary btn-sm mt-2">
            ...
            <div class="card-footer clearfix">
                ...
                <div class="float-right">
                    {{ $tramXe->appends(request()->query())->links() }}
                </div>
            </div>
            @elseif($tramXe->total() > 0)
            <div class="card-footer">
                <p class="text-muted mb-0">
                    Tổng số: {{ $tramXe->total() }} trạm xe
                    @if(request('search'))
                    <span class="badge badge-info ml-2">
                        <i class="fas fa-filter"></i> Đang lọc: "{{ request('search') }}"
                    </span>
                    @endif
                </p>
            </div>
