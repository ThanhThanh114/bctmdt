                        ...
                        <tbody>
                            @forelse($upcoming_trips as $booking)
                                <tr>
                                    <td>
                                        @php
                                            $ngayDi = $booking->chuyenXe->ngay_di instanceof \Carbon\Carbon
                                                ? $booking->chuyenXe->ngay_di->format('Y-m-d')
                                                : $booking->chuyenXe->ngay_di;
                                            $gioDi = is_string($booking->chuyenXe->gio_di)
                                                ? $booking->chuyenXe->gio_di
                                                : $booking->chuyenXe->gio_di->format('H:i:s');
                                        @endphp
                                        {{ \Carbon\Carbon::parse($ngayDi . ' ' . $gioDi)->format('d/m/Y H:i') }}
                                    </td>
                                    <td>
        ...
        <div class="col-md-4">
                ...
                <div class="card-body">
                    @forelse($popular_routes as $route)
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            <div>
                                <h6 class="mb-0">
                                    @if($route->chuyenXe && $route->chuyenXe->tramDi && $route->chuyenXe->tramDen)
                                        {{ $route->chuyenXe->tramDi->ten_tram }} - {{ $route->chuyenXe->tramDen->ten_tram }}
                                    @else
                                        Tuyến {{ $route->chuyen_xe_id }}
                                    @endif
                                </h6>
                                <small class="text-muted">{{ $route->trip_count }} lượt đặt</small>
                            </div>
                            @if($route->chuyenXe)
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <a href="{{ route('booking.history') }}" class="btn btn-success btn-block">
                    ...
                    <a href="{{ route('tracking.tracking') }}" class="btn btn-info btn-block">
                        <i class="fas fa-map-marker-alt mr-2"></i> Tra cứu vé
                    </a>
                </div>
            </div>
