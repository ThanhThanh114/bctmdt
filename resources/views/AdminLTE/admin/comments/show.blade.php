    ...
    <div class="col-md-8">
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-md-6">
                        <p><strong>Tuyến:</strong> {{ $comment->chuyenXe->tramDi->ten_tram }} → {{ $comment->chuyenXe->tramDen->ten_tram }}</p>
                        <p><strong>Ngày đi:</strong> {{ \Carbon\Carbon::parse($comment->chuyenXe->ngay_di)->format('d/m/Y') }}</p>
                    </div>
                    <div class="col-md-6">
                        <p><strong>Giờ đi:</strong> {{ date('H:i', strtotime($comment->chuyenXe->gio_di)) }}</p>
                        <p><strong>Loại xe:</strong> {{ $comment->chuyenXe->loai_xe }}</p>
                        <p><strong>Nhà xe:</strong> {{ $comment->chuyenXe->nhaXe->ten_nha_xe }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card comment-card">
            ...
            <div class="card-body">
                    ...
                    <div class="flex-grow-1">
                        ...
                        <div class="mt-2">
                            @for($i = 1; $i <= 5; $i++)
                                ...
                                @endif
                            @endfor
                            <span class="ml-2">({{ $comment->so_sao }}/5 sao)</span>
                        </div>
                    </div>
                </div>
                ...
                <small class="text-muted">
                    ...
                    {{ \Carbon\Carbon::parse($comment->ngay_bl)->format('d/m/Y H:i') }}
        ...
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('admin.comments.reply', $comment->ma_bl) }}" method="POST">
                    @csrf
    ...
    <div class="col-md-4">
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                @if($comment->ngay_duyet)
                <p class="mb-0">
                    <small class="text-muted">
                        Duyệt lúc: {{ \Carbon\Carbon::parse($comment->ngay_duyet)->format('d/m/Y H:i') }}
                    </small>
                </p>
                @endif
        ...
        <div class="card">
            ...
            <div class="card-body p-2">
                @if($comment->trang_thai == 'cho_duyet')
                <form action="{{ route('admin.comments.approve', $comment->ma_bl) }}" method="POST" class="mb-2">
                    @csrf
