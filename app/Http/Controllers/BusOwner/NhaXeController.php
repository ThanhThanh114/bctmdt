...
class NhaXeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Nếu là bus_owner, chỉ hiển thị nhà xe của họ
        if ($user->role === 'bus_owner' && $user->ma_nha_xe) {
        ...
        return view('AdminLTE.bus_owner.nha_xe.index', compact('nhaXes', 'statistics'));
    }
    ...
    {
        return view('AdminLTE.bus_owner.nha_xe.create');
    }
    ...
    public function show($id)
    {
        ...
        $user = Auth::user();
        if ($user->role === 'bus_owner' && $user->ma_nha_xe != $id) {
            return redirect()->route('bus-owner.nha-xe.index')
                ->with('error', 'Bạn không có quyền xem thông tin nhà xe này!');
        }
        ...
        return view('AdminLTE.bus_owner.nha_xe.show', compact('nhaXe', 'statistics'));
    }
    ...
    public function edit($id)
    {
        ...
        if ($user->role === 'bus_owner' && $user->ma_nha_xe != $id) {
            return redirect()->route('bus-owner.nha-xe.index')
                ->with('error', 'Bạn không có quyền chỉnh sửa nhà xe này!');
        }

        return view('AdminLTE.bus_owner.nha_xe.edit', compact('nhaXe'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($user->role === 'bus_owner' && $user->ma_nha_xe != $id) {
            return redirect()->route('bus-owner.nha-xe.index')
                ->with('error', 'Bạn không có quyền cập nhật nhà xe này!');
        }
        ...
        try {
            ...
            return redirect()->route('bus-owner.nha-xe.show', $id)
    ...
    public function destroy($id)
    {
        try {
            ...
            if ($user->role === 'bus_owner' && $user->ma_nha_xe != $id) {
                return redirect()->route('bus-owner.nha-xe.index')
                    ->with('error', 'Bạn không có quyền xóa nhà xe này!');
            }
